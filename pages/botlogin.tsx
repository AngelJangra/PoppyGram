...
export default function BotLogin(){
  ...
  async function finish(client:any,attempt:number){
    try{
      ...
      if(!r.ok||!j.ok){setSt('error');setMsg('Could not finalize ('+String(j.error||('HTTP '+r.status))+'). Return to the bot and try /auth again.');return}
      setSt('done');
  ...
  async function resendCode(){
    ...
    try{
      ...
      const apiHash=process.env.NEXT_PUBLIC_TG_API_HASH||'';
      const {StringSession}=await import('telegram/sessions');
      try{await win.__sa?.client?.disconnect?.().catch?.(()=>{})}catch{}
      const client=new mod.TelegramClient(new StringSession(''),apiId,apiHash,{connectionRetries:3,timeout:20});
      await client.connect();
      const res:any=await client.sendCode({apiId,apiHash},phone);
    ...
    }catch(e:any){setMsg('Could not resend: '+(String(e?.message||e?.errorMessage||'unknown')));}
  }
